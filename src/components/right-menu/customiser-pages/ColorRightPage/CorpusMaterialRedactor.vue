...
let selectedSurfaceID =
  useModelState().getCurrentModel.PROPS.CONFIG.MODULE_COLOR;
const materialList =
  useModelState().getCurrentModel.PROPS.CONFIG.MODULE_COLOR_LIST;
...
onMounted(() => {
  ...
  if (selectedSurfaceID) {
    let { NAME, DETAIL_PICTURE } = materialList.find(
      (material) => material.ID === selectedSurfaceID
    );
...
const onSearchChange = (e) => {
  ...
  let filtered = materialList.filter((material) =>
    reg.test(material.NAME.toLowerCase())
  );
...
const deleteSelectedOptions = (type: String) => {
  let { NAME, DETAIL_PICTURE, PREVIEW_PICTURE } = materialList[0];
      ...
      <div class="redactor">
        ...
        <ul class="redactor__list">
          <li
            ...
            @click="changeModuleTexture(material)"
          >
            <img
              class="item__img"
